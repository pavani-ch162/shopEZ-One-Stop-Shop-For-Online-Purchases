...
const Cart = () => {
  ...
  const fetchCart = async() =>{
    ...
    if(userId){
      await axios.get('http://localhost:6001/api/cart/fetch-cart').then(
        (response)=>{
  ...
  const increaseCartQuantity = async(id) =>{
    await axios.put('http://localhost:6001/api/cart/increase-cart-quantity', {id}).then(
      (response)=>{
        fetchCart();
      }
    ).catch((err)=>{
  ...
  const decreaseCartQuantity = async(id, quantity) =>{
    if(quantity > 1){
      await axios.put('http://localhost:6001/api/cart/decrease-cart-quantity', {id}).then(
        (response)=>{
          fetchCart();
        }
      ).catch((err)=>{
  ...
  const removeItem = async(id) =>{
      await axios.put('http://localhost:6001/api/cart/remove-item', {id}).then(
        (response)=>{
          fetchCart();
        }
      )
